### copied from https://github.com/ondrejbohdal/meta-calibration/blob/main/Metrics/metrics.py
...
class ClasswiseECELoss(nn.Module):
    ...
    def forward(self, logits, labels):
        num_classes = int((torch.max(labels) + 1).item()) ## change it for segmentation 
        softmaxes = F.softmax(logits, dim=1)  
        per_class_sce = None  
