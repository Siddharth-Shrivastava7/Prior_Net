...
class Trainer(BaseTrainer):
    ...
    def train(self):
        writer = SummaryWriter(comment="run_ccm")
            ...
            for epoch in range(self.config.epochs):
                ...
                for i_iter, batch in tqdm(enumerate(self.loader)):
                    ...
                    if i_iter % self.config.print_freq == 0:
                        self.print_loss(cu_step)
                        # valloss = self.validate2(cu_step)
                        # trainloss_s = self.losses['loss_source'].item()
                        # trainloss_e = self.losses['loss_entropy'].item()
                        
                        # iter_loss = 'iter_loss' + str(r)

                        # writer.add_scalars(iter_loss, {'train_loss_seg': trainloss_s,'train_loss_entropy':trainloss_e, 'val_loss_seg': valloss}, cu_step) 

                        # if(valloss < best_val_loss):
                        #     # miou = self.validate()
                        #     best_val_loss = valloss
                        #     print('********************************')
                        #     print('best_val_epoch_loss: ', best_val_loss)
                        #     print("MODEL UPDATED")
                        #     name = 'modelval' + str(r) + '.pth'
                        #     torch.save(self.model.state_dict(), osp.join(self.config["snapshot"], name))
                    
                    if i_iter % self.config.val_freq ==0 and i_iter!=0:
                        miou = self.validate() 
                        print('********************************')
                        name = 'modelval' + str(r) + '____' + str(i_iter) + '.pth'
                        torch.save(self.model.state_dict(), osp.join(self.config["snapshot"], name))
                        print('model saved')
    ...
    def save_model(self, iter):
        tmp_name = "_".join(("city2dz", str(iter))) + ".pth"
