...
class UNet16(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        print(conv5.shape) # torch.Size([2, 512, 67, 120])

        center = self.center(self.pool(conv5))
        print(center.shape)  # torch.Size([2, 256, 66, 120])
...
## there is a problem in upsampling layer size mis match in after center and conv5 layer. have to resolve if you need to use it further. similar type of problem also occured in the unet_resnet file so can look that for resolving this issue 