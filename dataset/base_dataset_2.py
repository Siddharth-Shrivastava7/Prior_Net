        ...
        try:
            ...
            if self.dataset in ['acdc_val_rf_tensor', 'acdc_train_rf_tensor', 'acdc_dz_val_rf_tensor']:
                ...
                if self.set == 'val': 
                    ...
                    label = TF.resize(label, (512, 512), interpolation = Image.NEAREST)   # extra for ..orginal evalutaion...
                    ...
                    label = Image.open(datafiles['label']) 
                    i, j, h, w = transforms.RandomCrop.get_params(  ## random crop exp
                                    label, output_size=(512, 512))
                    # print('****************')
                    # print(i,j,h,w)
                    ...
                    # label = TF.crop(label, i, j, h,w) 
                    # need to see if the value changes...ie is the 0,1,255 value in label changing after resize?
                    # tfr = transforms.Resize((512,512), interpolation = 'NEAREST') # interpolation by nearest neighbour...might preserve the original values. 
                    # print('************')
                    random.seed(seed) 
                    torch.manual_seed(seed)
                    # print(label) # <PIL.PngImagePlugin.PngImageFile image mode=L size=1920x1080 at 0x7FBA88CFDBD0>
